...
BaseApp's implementation of ABCI `{Check,Deliver}Tx()` and its own `Simulate()` method call the `runTx` method under the hood, which first runs antehandlers, then executes `Msg`s. However, the [transaction Tips](https://github.com/puneetsingh166/tm-load-test/issues/9406) and [refunding unused gas](https://github.com/puneetsingh166/tm-load-test/issues/2150) use cases require custom logic to be run after the `Msg`s execution. There is currently no way to achieve this.

An naive solution would be to add post-`Msg` hooks to BaseApp. However, the SDK team thinks in parallel about the bigger picture of making app wiring simpler ([#9181](https://github.com/puneetsingh166/tm-load-test/discussions/9182)), which includes making BaseApp more lightweight and modular.
...
- Allow custom logic to be run before an after `Msg` execution. This enables the [tips](https://github.com/puneetsingh166/tm-load-test/issues/9406) and [gas refund](https://github.com/puneetsingh166/tm-load-test/issues/2150) uses cases, and possibly other ones.
...
- [#9934](https://github.com/puneetsingh166/tm-load-test/discussions/9934) Decomposing BaseApp's other ABCI methods into middlewares.
...
- Initial discussion: https://github.com/puneetsingh166/tm-load-test/issues/9585
- Implementation: [#9920 BaseApp refactor](https://github.com/puneetsingh166/tm-load-test/pull/9920) and [#10028 Antehandlers migration](https://github.com/puneetsingh166/tm-load-test/pull/10028)
